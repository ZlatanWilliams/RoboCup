...
class PositioningState(FSMState):
    def execute(self, dt):
        """
        Handle player positioning, including special behavior during kickoff.
        """
        # Always move to tactical position if we're far from it
        if hasattr(self.player, 'target_position'):
            ...
            if distance_to_target > 0.1:
                # Key fix: Always seek target position
                seek(self.player, self.player.target_position, dt)
        
        # Special handling for kickoff
        if self._is_kickoff_state():
            self._handle_kickoff_positioning(dt)
            return
        
        # Regular play - check if we should go for the ball
        self._handle_regular_ball_pursuit(dt)
    
    def _is_kickoff_state(self):
        """
        Check if the game is currently in kickoff state.
        """
        # Check game state through team if available
        ...
        # Alternative check based on ball position
        if self.player.ball:
            if np.linalg.norm(self.player.ball.position) < 0.1:
                return True
        
        return False
    
    def _handle_kickoff_positioning(self, dt):
        """
        Handle player behavior during kickoff.
        """
        # Only the striker from the kickoff team should go for the ball
        is_kickoff_team = False
        kickoff_team = None
        
        # Get kickoff team from game engine if available
        if hasattr(self.player, 'team') and hasattr(self.player.team, 'game_engine'):
            kickoff_team = self.player.team.game_engine.kickoff_team
            is_kickoff_team = (self.player.team.team_side == kickoff_team)
            kickoff_delay_over = self.player.team.game_engine.kickoff_timer >= self.player.team.game_engine.kickoff_delay
        else:
            # Default to allowing kickoff if we can't check
            kickoff_delay_over = True
        
        # If we're the striker on the kickoff team, pursue the ball
        if is_kickoff_team and (self.player.role == 'striker' or self.player.role.startswith('striker')):
            if kickoff_delay_over:
                print(f"{self.player.role} going for kickoff!")
                self.player.fsm.change_state(PlayerState.PURSUE_BALL)
            else:
                # Move toward target position while waiting
                if hasattr(self.player, 'target_position'):
                    # Apply direct movement to ensure players are moving
                    seek(self.player, self.player.target_position, dt)
            
    def _handle_regular_ball_pursuit(self, dt):
        """
        Handle ball pursuit during regular play.
        """
        if self.player.ball is None:
            return
            
        ball_pos = self.player.ball.position
        my_pos = self.player.position
        
        # Skip if ball is owned by someone else
        if self.player.ball.owner and self.player.ball.owner != self.player:
            return
        
        # Get player's zone from team strategy
        if not hasattr(self.player.team, 'strategy') or not hasattr(self.player.team.strategy, 'zones'):
            return
        ...
        if self.player.role.startswith('striker'):
            # More aggressive pursuit for strikers
            ball_dist = np.linalg.norm(my_pos - ball_pos)
            can_reach = ball_dist < 3.0  # Longer pursuit range for strikers
            ...
            if self.player.team.team_side == 'home':
                is_in_opponent_half = ball_pos[0] > 0
            else:
                is_in_opponent_half = ball_pos[0] < 0
            
            # Strikers go for ball if it's:
            # 1. In opponent half, OR
            # 2. Within extended pursuit range
            if (is_in_opponent_half or can_reach) and not self.player.ball.owner:
                print(f"Striker {self.player.role} pursuing ball!")
                self.player.fsm.change_state(PlayerState.PURSUE_BALL)
                
        elif 'defender' in self.player.role:
            # More active pursuit for defenders in own half
            ball_dist = np.linalg.norm(my_pos - ball_pos)
            can_reach = ball_dist < 2.5
            
            # Check if ball is in our half
            is_in_our_half = False
            if self.player.team.team_side == 'home':
                is_in_our_half = ball_pos[0] < 0
            else:
                is_in_our_half = ball_pos[0] > 0
            
            # Defenders aggressively pursue ball in our half or within range
            if (is_in_our_half and ball_dist < 3.0) or can_reach:
                if not self.player.ball.owner:
                    print(f"Defender {self.player.role} pursuing ball!")
                    self.player.fsm.change_state(PlayerState.PURSUE_BALL)
                    
        else:
            # Standard behavior for other players
            # Check if ball is in player's zone
            ball_in_zone = (
                zone["min_x"] <= ball_pos[0] <= zone["max_x"] and 
